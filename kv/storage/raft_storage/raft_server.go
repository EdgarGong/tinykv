...
func (rs *RaftStorage) checkResponse(resp *raft_cmdpb.RaftCmdResponse, reqCount int) error {
	// Maybe the information is wrong or stale,
	// so raftstore needs to check them and decide
...
func (rs *RaftStorage) Write(ctx *kvrpcpb.Context, batch []storage.Modify) error {
	// ctx carries the Region information
	// from the perspective of the client and
	// is passed as the header of RaftCmdRequest
...
func (rs *RaftStorage) Reader(ctx *kvrpcpb.Context) (storage.StorageReader, error) {
	...
	request := &raft_cmdpb.RaftCmdRequest{
		Header: header,
		Requests: []*raft_cmdpb.Request{{ // only 1 Request
